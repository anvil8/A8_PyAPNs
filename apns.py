...
class APNsConnection(object):
    ...
    def _connect(self):
        ...
        _logger.debug("%s APNS connection establishing..."
        ...
        if self.enhanced:
            ...
            self._ssl = wrap_socket(self._socket, self.key_file,
                                    self.cert_file,
        ...
        _logger.debug("%s APNS connection established"
                      % self.__class__.__name__)
    ...
    def disconnect(self):
        if self.connection_alive:
            ...
            if self._ssl:
                self._ssl.close()
            self.connection_alive = False
            _logger.info(" %s APNS connection closed"
                         % self.__class__.__name__)
    ...
    def write(self, string):
        if self.enhanced:  # nonblocking socket
            ...
            _, wlist, _ = select.select(
                [], [self.connection()], [], WAIT_WRITE_TIMEOUT_SEC
            )

            if len(wlist) > 0:
                length = self.connection().sendall(string)
                if length == 0:
                    _logger.debug("sent length: %d" % length)  #DEBUG
            else:
                _logger.warning("write socket descriptor is not ready after "
                                + str(WAIT_WRITE_TIMEOUT_SEC))
...
class Payload(object):
    ...
    def json(self):
        return json.dumps(
            self.dict(), separators=(',', ':'), ensure_ascii=False
        ).encode()
...
class Frame(object):
    ...
    def add_item(self, token_hex, payload, identifier, expiry, priority):
        ...
        self.notification_data.append({'token': token_hex, 'payload': payload,
    ...
    def get_notifications(self, gateway_connection):
        notifications = list(
            {
                'id': x['identifier'],
                'message': gateway_connection.get_enhanced_notification(
                    x['token'], x['payload'], x['identifier'], x['expiry']
                )
            }
...
class GatewayConnection(APNsConnection):
    ...
    @staticmethod
    def _get_notification(token_hex, payload):
        """
    ...
    @staticmethod
    def get_enhanced_notification(token_hex, payload, identifier, expiry):
        """
    ...
    def send_notification(self, token_hex, payload, identifier=0, expiry=0):
        """
        in enhanced mode, send_notification may return error response
        from APNs if any
        """
        if self.enhanced:
            self.last_activity_time = time.time()
            message = self.get_enhanced_notification(token_hex, payload,
                                                     identifier, expiry)
            ...
            for i in range(WRITE_RETRY):
                try:
                    with self.send_lock:
                        ...
                        self.write(message)
                        self._sent_notifications.append({'id': identifier,
                                                         'message': message})
                    break
                except socket_error as e:
                    delay = 10 + (i * 2)
                    _logger.exception("sending notification with id:"
                                      + str(identifier)
                                      + " to APNS failed: " + str(type(e))
                                      + ": " + str(e) + " in " + str(i+1)
                                      + "th attempt, will wait " + str(delay)
                                      + " secs for next action")
                    time.sleep(delay)  # wait for error-response to be read
    ...
    def registerresponse_listener(self, response_listener):
    ...
    def is_idle_timeout(self):
        timeout_idle = 30
        return (time.time() - self.last_activity_time) >= timeout_idle
    ...
    class ErrorResponseHandlerWorker(threading.Thread):
        ...
        def run(self):
            while True:
                ...
                if self._apns_connection.is_idle_timeout():
                    idled_time = (time.time()
                                  - self._apns_connection.last_activity_time)
                ...
                try:
                    rlist, _, _ = select.select(
                        [self._apns_connection.connection()], [], [], 
                        WAIT_READ_TIMEOUT_SEC
                    )

                    if len(rlist) > 0:  # there's some data from APNs
                        with self._apns_connection.send_lock:
                            ...
                            if len(buff) == ERROR_RESPONSE_LENGTH:
                                ...
                                if 8 == command:  # error response from APNS
                                    error_response = (status, identifier)
                                    if self._apns_connection.response_listener:
                                        self._apns_connection.response_listener(
                                    ...
                                    self._apns_connection.disconnect()
                                    self._resend_notifications_by_id(
                                        identifier
                                    )
                            if len(buff) == 0:
                ...
                time.sleep(0.1)  # avoid crazy loop if something bad happened.
                # e.g. using invalid certificate

            self._apns_connection.disconnect()
            _logger.debug("error-response handler worker closed")  # DEBUG

        def _resend_notifications_by_id(self, failed_identifier):
            fail_idx = Util.get_list_index_from_id(
            ...
            self._resend_notification_by_range(
                fail_idx+1, len(self._apns_connection._sent_notifications)
            )
            return
        ...
        def _resend_notification_by_range(self, start_idx, end_idx):
            ...
            for sent_notification in self._apns_connection._sent_notifications:
                ...
                try:
                    ...
                    _logger.exception(
                        "resending notification with id:"
                        + str(sent_notification['id'])
                        + " failed: " + str(type(e)) + ": " + str(e)
                    )  # DEBUG
                    break
...
class Util(object):
    @classmethod
    def get_list_index_from_id(cls, the_list, identifier):
